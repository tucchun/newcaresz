...
define(['jquery'],function($){
    ...
    function up_down (ele,type){
        ...
        if(ele.hasClass('icon-up')){
            ...
            if((index-0+1) === alen){
                ...
                $('<i class="icon icon-down"></i>').insertAfter(_obj.find('.icon-up'));
            }
            ...
            if(index === 0){
                $('<i class="icon icon-up"></i>').insertBefore(_obj.find('.icon-down'));
    ...
    function add(ele){
        var /*tar_obj = ele.parents('.sort_wrap').clone(true),*/
            len = ele.parents('.sort_wrap').parent().find('.sort_wrap').length,
