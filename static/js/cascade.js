define([
	...
	function Cascade(opts){
		...
		// 渲染级联元素
		this.render(this.$primary, currentPrimaryData, this.defaultPrimary);
		var currentSecondaryData = _.filter(this.secondaryData, function(item){
			return item[this.parentId] === this.$primary.val();
		}.bind(this));
		this.render(this.$secondary, currentSecondaryData, this.defaultSecondary);
	...
	$.extend(Cascade.prototype, {
		render: function($select, data, defaultCode){
		...
		bindEvent: function(){
			...
			this.$primary.on("change", function(e){
				...
				self.render(self.$secondary, currentSecondaryData);
			});
		}
	});
